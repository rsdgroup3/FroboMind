...
void frobit_callback(const fmMsgs::nmea& msg) {
	...
	if (msg.id == "RC") {
		...
		if(msg.type == "ADC") {
			//build first message
	//		frobit_adc.value.clear();
			frobit_adc.header.stamp = msg.header.stamp;
			frobit_adc.header.frame_id = "left";
			frobit_adc.value0 = ( ( (double) boost::lexical_cast<int>( msg.data[0] ) / 255.0 ) * 5.0 );
			frobit_adc.value1 =( ( (double) boost::lexical_cast<int>( msg.data[1] ) / 255.0 ) * 5.0 );
