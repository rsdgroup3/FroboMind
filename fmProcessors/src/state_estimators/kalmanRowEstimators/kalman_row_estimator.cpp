...
public:
	...
	void processRowMeasurementUpdate(const fmMsgs::rowConstPtr& row_msg) {
...
int main(int argc, char **argv) {
	...
	row_s = nh.subscribe<fmMsgs::row>(row_s_t.c_str(),1, &KalmanRowEstimator::processRowMeasurementUpdate,&krf);


	krf.row_pub = nh.advertise<fmMsgs::row>(row_est_p_t,1);
