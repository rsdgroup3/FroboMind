...
public:
	...
	RoboTeq( int channel )
	{
		...
		{
			ROS_WARN("motor channel not in range!");
			motor_channel = 1;
		}
		deadman_active = false;
		cmd_vel_active = false;
	...
	void onSerialMsgReceived(const fmMsgs::serial::ConstPtr& msg)
	{
		...
		double version=0;
		int cbr_rel = 0;
		float motor_amps_in_ch1,motor_amps_in_ch2,motor_amps_out_ch1,motor_amps_out_ch2;
		...
		else if(sscanf(msg->data.c_str(),"A=%f:%f",&motor_amps_out_ch1,&motor_amps_out_ch2))
		{
			if( motor_channel == 1)
				m.motor_amps_out = motor_amps_out_ch1;
			else if ( motor_channel == 2)
				m.motor_amps_out = motor_amps_out_ch2;
		}
		else if(sscanf(msg->data.c_str(),"BA=%f:%f",&motor_amps_in_ch1,&motor_amps_in_ch2))
		{
			if( motor_channel == 1)
				m.motor_amps_in = motor_amps_in_ch1;
			else if ( motor_channel == 2)
				m.motor_amps_in = motor_amps_in_ch2;
		}
	...
	void spin(const ros::TimerEvent& e)
	{
		if(initialised)
		{
			...
			{
			case 0:
				ss << "!G " << (int)motor_channel << " "<< (int)(velocity * twist_to_motor_velocity) << "\r";
				break;
			case 1:
				ss << "?CBR\r";
				break;
...
int main(int argc,char** argv)
{
	...
	nh.param<int>("motor_channel",channel,1);
